...
st.title('Theft Data Dashboard')
crimes = st.multiselect('Select crimes:', options=df['Crime'].unique(), default=df_brooklyn_sorted['Crime'].unique())
filtered_df_2 = df_brooklyn_sorted[df_brooklyn_sorted['Crime'].isin(crimes)]
fig = px.line(filtered_df_2, x='Start_date', y='Current_total', color='Crime', title='Theft Total Over Time')
st.plotly_chart(fig)
