...
# df_quarterly = df_weekly.resample('Q').sum()
df_brooklyn_indexed = df_brooklyn.set_index('Start_date')
df_brooklyn_quarterly = df_brooklyn_indexed["Current_total"].resample('Q').sum().reset_index()
df_brooklyn_quarterly_sorted = df_brooklyn_quarterly.sort_values(by="Start_date",ascending=True)
df_brooklyn_sorted = df_brooklyn.sort_values(by="Start_date", ascending=False)

# print(df_brooklyn_quarterly_sorted)
...
html.Div([
        ...
        html.P("I got the data from BetaNYC on crime in NYC through APIs. It has a weekly numbers for each precinct in NYC. For example, “Petit Larceny” week 7/24/2024 - 7/30/2024 34. I cleaned up the dates for some of the weeks. Then I zoom in into South and North Brooklyn, and merge data into a data frame. First, I want to see if there is more crime in Summer than in Winter months in Brooklyn in 2023. Generally, it was true there was more crime in Summer than in Winter in 2023 except Rape and Murder. I then checked if it's true for 2022. It was not. Some crimes happened more frequently in Summer, others in Winter. Summer vs Winter 2022 crimes were inconsistent with Summer vs Winter 2023.", style={'color': 'white'}),
        html.P("Second, I decided to look into 4 types of what I called “theft”: Petit Larceny, Grand Larceny, Burglary, and Robbery. Petit Larceny is the most frequent of these four in Brooklyn in 2022-2024 which makes sense because it's the least risky. I was surprised that Burglary and Robbery happened almost at the same rate. Robbery is much more risky, has a bigger punishment, involves interaction with a victim and “threat of violance”. Anything can happen. Burglary is less risky, does not require interaction with a victim, and has small punbishment than robbery. It is surprising that robbery and burglary happened almost at the same rate. Sometimes, burglary happened less often. Third, I created an interactive dashboard using Dash that allows a user to pick a type of crime and between 2022 and early 2024. I found out that NYC police has a website that shows all this data in an interactive dashboard with more detailed data than mine except there is no filter for dates, only most recent week.", style={'color': 'white'}),
        html.P("Third, I created an interactive dashboard using Dash that allows a user to pick a type of crime and between 2022 and early 2024. I found out that NYC police has a website that shows all this data in an interactive dashboard with more detailed data than mine except there is no filter for dates, only most recent week.", style={'color': 'white'})
    ], style={'backgroundColor': 'black', 'padding': '10px'})
...
def update_line_chart(selected_crimes,start_date, end_date):
    ...
    color_sequence = ['#636EFA', '#EF553B', '#00CC96', '#AB63FA', '#FFA15A', '#19D3F3', '#FF6692', '#B6E880']
    line_fig = px.line(filtered_df_2, x='Start_date', y='Current_total', color='Crime', title='Crime in Brooklyn',color_discrete_sequence=color_sequence)

    bar_fig = px.bar(df_brooklyn_quarterly_sorted, x='Start_date', y='Current_total', color='Current_total', title='Quarterly Crime in Brooklyn',
                  color_discrete_sequence=color_sequence)


    return line_fig,treemap_fig,bar_fig
if __name__ == '__main__':
